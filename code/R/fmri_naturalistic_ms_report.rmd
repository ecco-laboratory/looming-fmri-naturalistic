...
```{r figure-1e-1}
knitr::include_graphics(tar_read(fig_ms_schematic_unit.activation, store = target_store), rel_path = FALSE)
```

```{r figure-1e-2}
knitr::include_graphics(tar_read(fig_ms_schematic_encoding.bold.pred, store = target_store), rel_path = FALSE)
```
...
tar_read(pattern.discrim.controlled_alexnet, store = target_store) %>% 
  yardstick::metrics(truth = animal_type, estimate = animal_type_pred, starts_with("outcome")) %>% 
  mutate(across(where(is.numeric), \(x) signif(x, digits = 3)))
```
...
### full & partial correlations of encoding patterns predicting self-report

### partial correlations of encoding patterns predicting self-report

```{r}
summary_encoding.selfreport_combined <- tar_read(summary_encoding.selfreport, store = target_store) %>% 
  full_join(tar_read(summary_encoding.selfreport_pcor, store = target_store),
            by = c("data_subset", "rating_type", "model_type")) %>% 
  # drop the bootstrap distribution SDs (aka bootstrapped SEs) bc keeping the 95% CIs which are from the same distribution anyway
  select(-ends_with("sd")) %>% 
  mutate(across(where(is.numeric), \(x) signif(x, digits = 3)))
```

```{r}
summary_encoding.selfreport_combined %>% 
...
Option 1, new as of 2025-06-30

```{r figure-4b-option-1}
knitr::include_graphics(tar_read(fig_ms_encoding.selfreport.combined, store = target_store), rel_path = FALSE)
```

Option 2, a figure of this form was on the SANS 2025 poster

```{r figure-4b-option-2}
