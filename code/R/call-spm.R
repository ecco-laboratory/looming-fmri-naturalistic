...
spm_spec_est_level1 <- function (model_path,
  ...
  matlab_commands = c(
    ...
    # contrasts are set by the matlab script path assigned to script_contrast 
    # the main matlab script calls that script
    # we only set script_contrast as an argument so that targets will track the dependency
    call_script(script)
  )
